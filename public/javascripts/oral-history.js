	...
	var _ui_state = { stat_zoom: false, history : [], step: 0}
	...
	function initializeVideoDialog(callback) {
		...
		for (key in videoInfo) {
           // console.log(videoInfo[key]);
	        ...
	        cloned.find('a').click(function() {return false;});
...
//                    console.log(event);
            ...
            var video_lenght = popcorn.duration(); //videoInfo[currentId).length;
...
*/	
	...
	function saveEvent() {
	    ...
	    var eventData = { 'vid_id': _ui_state.history[_ui_state.step-1]._id,'vid_title': _ui_state.history[_ui_state.step-1].title,'start-time': $('#start-time').val(), 'end-time': $('#end-time').val(), //$('#text-comments').val(),
